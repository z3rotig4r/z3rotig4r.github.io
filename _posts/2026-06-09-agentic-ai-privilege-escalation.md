...
## 핵심 명제: 에이전트의 도구 권한 = 공격표면

전통 앱에서 권한 상승은 보통 버그(예: 경계 검사 누락)를 악용한다. 에이전트에서는 다르다. **에이전트에 부여한 도구 하나하나가 그대로 공격표면**이 된다. 모델은 "작업을 효율적으로 끝내기 위해" 가진 도구를 자율적으로 고른다 — 그 판단이 공격자의 주입된 명령에 휘둘리면, 권한은 설계자가 의도한 선을 넘는다.
...
- 에이전트의 **도구 권한이 곧 공격표면**. 인젝션이 도구를 만나면 권한 상승으로 번진다.
