...
- **OG 커버 없음 → `image-designer` 호출**: front matter에 `image.path`가 없으면 발행 직전 `og-image` 스킬로 커버 생성·주입(`OPENAI_API_KEY` 로컬 필요). 키 없으면 OG는 WARN으로 두고 진행, 사용자에 키 요청.
- 뉴스 다이제스트: `linkcheck.py`로 외부 링크 전수 200 확인 — 실패 링크 항목 제외.
- `seo-lint` FAIL → seo-optimizer로 반려.
- `editor-reviewer` 판정 `BLOCK`/`CHANGES_REQUESTED` → 해당 에이전트 반려.
- `bundle exec jekyll b` + htmlproofer 실패 → 통과까지 차단(빈 image 블록·깨진 `/{` 금지).
...
```
news-scout ─┐
            ├─→ post-writer ─→ seo-optimizer ─→ image-designer(OG) ─→ editor-reviewer ─→ (APPROVE) ─→ 사람 PR 머지
wiki-publisher ┘                                                          │
                                                              (CHANGES) ─┘ 반려→해당 에이전트
```
