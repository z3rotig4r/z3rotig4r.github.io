...
## GEO 구조 — AI 검색 인용 + CTR (필수)
생성형 엔진(ChatGPT·Perplexity·Gemini·구글 AI Overview)에 인용되게 쓴다. 신생 사이트가 증거 신호로 역전하는 채널(Princeton GEO, KDD 2024).
- **PAWC — 첫 150단어에 직답:** TL;DR 박스 + 첫 문단에 정의·핵심 수치를 두괄식으로. AI는 첫 문장을 ~5배 가치로 인용 → "왜 중요한지"만 끌지 말고 **답을 먼저**.
- **증거밀도 > 키워드밀도:** 단위 있는 수치·1차 출처·named individual 직접 인용을 넉넉히. 키워드 반복은 오히려 해(−8%). 통계+유려함이 최강(≥+35%).
- **FAQ 섹션(정보성 글 권장):** 글 끝 `## 자주 묻는 질문` + **질문형 H3** 3~5개. 동시에 front matter `faq:` 리스트(`- q:`/`a:`)에 같은 내용 → `_includes/metadata-hook.html`가 FAQPage JSON-LD 자동 생성(리치결과). 본문 Q&A와 front matter 내용 일치.
